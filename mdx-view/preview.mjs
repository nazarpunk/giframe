...
import {parse as parseMDL} from './mdl/parse.mjs';
...
function initDragDrop() {
    ...
    const dropTexture = (file, textureName, textureFlags) => {
            ...
            reader.onload = () => {
                try {
                    if (isDDS) {
                        ...
                        if (format) {
                            modelRenderer.setTextureCompressedImage(textureName, format, reader.result, dds, textureFlags);
                        } else {
                        ...
                        const blp = decode(reader.result);
