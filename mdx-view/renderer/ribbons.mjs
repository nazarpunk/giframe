import {getShader} from './util.mjs';
import {ModelInterp} from './modelInterp.mjs';
import {FilterMode, LayerShading} from '../model.mjs';
import {vec3} from './../gl-matrix/index.mjs';
...
export class RibbonsController {
    ...
    constructor(model, interp, rendererData) {
        ...
        if (this.model.RibbonEmitters.length) {
            for (const ribbonEmitter of this.model.RibbonEmitters) {
                ...
                emitter.baseCapacity = Math.ceil(ModelInterp.maxAnimVectorVal(emitter.props.EmissionRate) * emitter.props.LifeSpan) + 1; // extra points
    ...
    appendVertices(emitter) {
        const first = vec3.clone(emitter.props.PivotPoint);
        const second = vec3.clone(emitter.props.PivotPoint);
